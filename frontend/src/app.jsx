...
function DesignerCurricula({ notify }) {
    const own = mockCurricula.filter((item) => item.designer === 'Ananya Iyer');
    const [, setLocation] = useLocation();
    return <div className="content"><PageHeader eyebrow="Curriculum Designer" title="My curricula" detail="Build with the official record in view, then submit when the evidence is ready." action={<button className="btn btn-primary" onClick={() => setLocation('/designer/create')} data-testid="button-create-curriculum"><Plus size={15}/> Create curriculum</button>}/><div style={{ display: 'grid', gap: 13 }}>{own.map((item, i) => <div className="surface" style={{ padding: 19 }} key={item.id}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 15, alignItems: 'flex-start' }}><div><span className="eyebrow">{item.program} · v{item.version}</span><h2 className="font-display" style={{ fontSize: 18, margin: '7px 0' }}>{item.name}</h2><p className="muted" style={{ fontSize: 11, margin: 0 }}>Last submitted {item.submitted} · {item.score}/100 screening score</p></div><StatusBadge status={item.status}/></div><div style={{ display: 'flex', gap: 8, marginTop: 17 }}><button className="btn" onClick={() => notify('Curriculum editor opened with governed sections.')} data-testid={`button-edit-own-curriculum-${i}`}><Pencil size={14}/> Open editor</button><button className="btn" onClick={() => notify('Curriculum submitted to the reviewer queue.')} data-testid={`button-submit-own-curriculum-${i}`}><Send size={14}/> Submit version</button></div></div>)}</div></div>;
}
function CreateWizard({ notify }) {
    ...
    const content = [
        <div className="form-grid" key="basic"><div className="upload-field" style={{ gridColumn: '1 / -1' }}><div><label className="label">Curriculum PDF</label><span className="muted" style={{ fontSize: 11 }}>Upload the working curriculum source. PDF only, up to 25 MB.</span></div><label className="btn btn-primary upload-button"><Upload size={14}/> Choose PDF<input type="file" accept=".pdf,application/pdf" data-testid="input-curriculum-pdf"/></label></div><Field label="Program" value="B.Tech Artificial Intelligence"/><Field label="Degree" value="Bachelor of Technology"/><Field label="Branch" value="Artificial Intelligence & Data Science"/><Field label="Academic year" value="2026–27"/><Field label="Version" value="2.2"/><Field label="Duration" value="4 years / 8 semesters"/><div style={{ gridColumn: '1 / -1' }}><label className="label">Description</label><textarea className="textarea" defaultValue="A practice-led undergraduate curriculum for responsible AI, data systems and intelligent products." data-testid="textarea-curriculum-description"/></div></div>,
    ...
    return <div className="content"><PageHeader eyebrow="Curriculum Designer / Builder" title="Create curriculum" detail="A single academic editor for structure, outcomes, evidence and review readiness." action={<span className="badge badge-lavender">Draft autosaved</span>}/>{saved ? <div className="surface" style={{ padding: 40, textAlign: 'center' }}><CheckCircle2 size={34} color="#50865d"/><h2 className="font-display" style={{ margin: '12px 0 5px' }}>Submitted for screening</h2><p className="muted" style={{ fontSize: 12 }}>B.Tech Artificial Intelligence v2.2 is now in the governed review workflow.</p></div> : <div className="surface" style={{ padding: 21 }}><div style={{ display: 'flex', gap: 7, overflowX: 'auto', paddingBottom: 18, marginBottom: 18, borderBottom: '1px solid hsl(var(--border))' }}>{steps.map((item, i) => <button key={item} className={`btn ${i === step ? 'btn-primary' : 'btn-ghost'}`} style={{ whiteSpace: 'nowrap', padding: '0 10px' }} onClick={() => setStep(i)} data-testid={`button-wizard-step-${i}`}><span style={{ opacity: .7 }}>{i + 1}</span> {item}</button>)}</div><div style={{ minHeight: 260 }}>{content[step]}</div><div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, paddingTop: 16, borderTop: '1px solid hsl(var(--border))' }}><button className="btn" disabled={step === 0} onClick={() => setStep(Math.max(0, step - 1))} data-testid="button-wizard-back"><ArrowLeft size={14}/> Back</button><button className="btn btn-primary" disabled={step === steps.length - 1} onClick={() => setStep(Math.min(steps.length - 1, step + 1))} data-testid="button-wizard-next">Continue <ArrowRight size={14}/></button></div></div>}</div>;
}
function Field({ label, value }) { return <div><label className="label">{label}</label><input className="field" defaultValue={value} data-testid={`input-wizard-${label.toLowerCase().replaceAll(' ', '-')}`}/></div>; }
